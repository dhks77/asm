...
- **IDE 실행** -- Ctrl+E로 워크트리를 IntelliJ/VSCode/Cursor 등에서 바로 열기 (config로 커스텀 추가)
...
**빌트인**: `intellij` (`idea`), `vscode` (`code`), `cursor` (`cursor`).
해당 CLI 가 `$PATH` 에 있어야 동작 (JetBrains Toolbox, VSCode `code`
명령, Cursor 각각 설치 + 등록 필요).

**커스텀 IDE / 오버라이드**: config 에 `[ides.<name>]` 섹션 추가.

```toml
# 기본 IDE 고정 (selector 건너뜀). "(none)" 이면 매번 selector.
default_ide = "intellij"

# 빌트인 오버라이드 — JetBrains Toolbox 없이 macOS 앱 실행
[ides.intellij]
command = "open"
args = ["-na", "IntelliJ IDEA CE.app"]
