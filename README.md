...
- **Tracker 연동** -- branch 이름에서 task title / URL 을 해석해 picker, launcher, worktree dialog 검색에 반영한다. tracker는 repo별 local config를 따르며 `(none)` 으로 비활성화할 수 있다.
...
- **Settings Scope** -- global user config와 project-local config를 함께 다루며, repo/worktree는 main repo 단위로 local provider / tracker / IDE 기본값을 공유한다.
...
- `local` 에서는 provider / tracker / default IDE override 와 repo color, worktree template 충돌 정책, worktree base path를 편집한다. repo-backed target에서만 보인다.
- `Default Tracker` 는 `(none)` 을 지원한다. 비우면 task title / URL resolve를 하지 않는다.
- `Ctrl+R` 은 현재 local override를 지우고 global 값을 다시 상속한다.
- `Ctrl+X` 는 현재 IDE 항목을 삭제하거나 built-in IDE override를 reset 한다.
- `Enter` 또는 `Ctrl+S` 로 저장한다. 저장/플러그인 설정 실패는 dialog 안에서 바로 에러로 보여준다.
...
- provider 이름은 registry 안에서 유일해야 한다. built-in (`claude`, `codex`) 과 같은 이름을 쓰면 로드가 거부된다.
...
#     "resume_args":["--resume"],
#     "needs_content":true,
#     "cmux_notification_hook":"claude-hook"
#   }

# cwd 기준 동적 resume 판단 (선택)
<plugin> resume-args /path/to/project
# → ["--resume"]

# pane title 기준 content capture 필요 여부 판단 (선택)
<plugin> needs-content "pane title"
# → true
...
빌트인 tracker는 `dooray` 이고, 커스텀 tracker도 `~/.asm/trackers/` 아래 실행파일로 둘 수 있다.

- tracker 이름은 settings 의 `Default Tracker` 에서 고른다.
- `(none)` 을 선택하면 tracker를 완전히 끈다.
- repo/worktree마다 local `default_tracker` / `[trackers.<name>]` 설정이 따로 적용된다.
...
- dialog는 먼저 현재 branch 목록으로 뜨고, background에서 best-effort `git fetch --all` 후 새 목록으로 갱신된다.
...
default_tracker = ""             # 비우면 tracker 비활성화
...
default_tracker = ""             # project-local tracker override 가능, "" = none
default_ide = "cursor"
