...
- **Launch Anywhere** -- `asm` 을 아무 경로에서나 실행해도 된다. `--path` 는 launcher 초기 위치, tmux session identity, project-local config 해석 기준이 된다.
- **Picker = Open Session List** -- 좌측 picker는 현재 asm 인스턴스에서 이미 열린 target만 보여준다.
- **Launcher = Open New Targets** -- 새 target은 `Ctrl+N` launcher에서 연다. top-level 탭 순서는 `Favorites`, `Directories`, `Recent` 이다.
- **Repo Drill-down** -- favorite repo 또는 repo directory를 고르면 main repo / linked worktree 목록으로 바로 들어간다.
- **멀티 인스턴스 안전성** -- tmux session 이름이 absolute path 기반으로 파생돼 서로 다른 rootPath의 asm을 동시에 띄울 수 있다.
...
이전 snapshot 이 있으면 AI / terminal 세션 수를 보여준 뒤 복원 여부를 묻는다.
