...
// PluginBacked providers originate from an external plugin executable.
type PluginBacked interface {
	PluginPath() string
}

// CMUXHookProvider can opt into a provider-specific cmux notification hook.
type CMUXHookProvider interface {
	CMUXNotificationHook() string
}
