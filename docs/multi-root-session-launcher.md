...
v1의 top-level launcher 탭 순서는 다음과 같다.

1. `Favorites`
2. `Directories`
3. `Recent`

repo browse/worktree drill-down은 `Favorites` 와 `Directories` 안에서 진입한다.

#### Favorites

즐겨찾은 dir / repo 목록.

- repo favorite 선택 시 main repo / linked worktree 목록으로 drill-down
- dir favorite 선택 시 해당 디렉토리 브라우저로 drill-down
...
| `ui/launcher_dialog.go` (신규) | `Favorites / Directories / Recent` launcher 구현 |
...
2. `Favorites / Directories / Recent` 모드 구현
