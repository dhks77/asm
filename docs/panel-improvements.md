...
**현재 동작**: picker visibility는 `Ctrl+L`로만 수동 토글한다. 새 세션을 열거나 회전해도 pane 상태를 자동으로 바꾸지 않는다.
...
| Ctrl+G | picker ↔ working focus toggle | 동작 변경 |
| Enter | 세션 열기 | 동작 변경 |
| 그 외 | — | 기존과 동일 |
...
| 파일 | 변경 내용 |
|------|----------|
| `config/config.go` | picker 관련 config 정리 |
| `ui/settings_dialog.go` | picker visibility 수동 토글 기준으로 settings 단순화 |
